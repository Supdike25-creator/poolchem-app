...
import { SidebarNav, buildGuardNavItems, buildManagerNavItems, mainNavItems } from './SidebarNav';
import {
...
function AuthShellStandardLayout({
  role,
  profile,
  error,
  sidebarItems,
  ...
  error: string;
  sidebarItems?: typeof mainNavItems;
  children: ReactNode;
  onBackToLogin: () => void;
  onLogout: () => void;
}) {
  return (
    <AuthShellFrame
      ...
      onLogout={onLogout}
      onBackToDev={() => undefined}
      sidebar={(
        <SidebarNav
          items={sidebarItems ?? mainNavItems}
          header={<AuthShellSidebarHeader role={role} />}
          footer={<AuthShellRoleFooter role={role} />}
        />
      )}
    >
      {children}
...
function AuthShellSidebarHeader({ role }: { role: AppRole }) {
  return (
...
function AuthShellRoleFooter({ role }: { role: AppRole }) {
  return (
...
export default function AuthShell({ role, children }: { role: AppRole; children: React.ReactNode }) {
  ...
  if (isDevPreview) {
    return <DevPerspectiveAuthShell>{layout}</DevPerspectiveAuthShell>;
  }
